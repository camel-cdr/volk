...
static inline void
volk_64u_byteswap_rvv(uint64_t* intsToSwap,
                      unsigned int num_points)
{
    size_t n = num_points;
    ...
    if (vlmax <= 256) {
        vuint8m1_t vidx = __riscv_vreinterpret_u8m1(__riscv_vsub(
                __riscv_vreinterpret_u64m1(__riscv_vid_v_u8m1(vlmax)), 0x0706050403020100-0x1020304050607, vlmax/8));
        for (size_t vl; n > 0; n -= vl, intsToSwap += vl) {
            vl = __riscv_vsetvl_e64m8(n);
            vuint8m8_t v = __riscv_vreinterpret_u8m8(__riscv_vle64_v_u64m8(intsToSwap, vl));
...
static inline void
volk_64u_byteswap_rva23(uint64_t* intsToSwap,
                        unsigned int num_points)
{
    size_t n = num_points;
    for (size_t vl; n > 0; n -= vl, intsToSwap += vl) {
