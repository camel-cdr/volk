...
static inline void
volk_8ic_x2_s32f_multiply_conjugate_32fc_rvv(lv_32fc_t* cVector,
                                             const lv_8sc_t* aVector,
                                             const lv_8sc_t* bVector,
                                             const float scalar,
                                             unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, aVector += vl, bVector += vl, cVector += vl) {
        ...
        vint16m2_t vi = __riscv_vsub(__riscv_vwmul(vai, vbr, vl), __riscv_vwmul(var, vbi, vl), vl);
                                                ...
                                                unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, aVector += vl, bVector += vl, cVector += vl) {
        ...
        vfloat32m4_t vrf = __riscv_vfmul(__riscv_vfwcvt_f(vr, vl), 1.0/scalar, vl);
        vfloat32m4_t vif = __riscv_vfmul(__riscv_vfwcvt_f(vi, vl), 1.0/scalar, vl);
        __riscv_vsseg2e32_v_f32m4x2((float*)cVector, __riscv_vcreate_v_f32m4x2(vrf, vif), vl);
    }
}
