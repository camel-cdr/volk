...
static inline void
volk_32f_8u_polarbutterfly_32f_rvv(float* llrs,
                                   unsigned char* u,
                                   const int frame_exp,
                                   const int stage,
                                   const int u_num,
                                   const int row)
{
    ...
    if (row) { // not necessary for ZERO row. == first bit to be decoded.
        ...
        size_t n = stage_size;
        for (size_t vl; n > 0; n -= vl, u_target += vl, src_llr_ptr += vl*2, dst_llr_ptr += vl) {
    ...
    while (min_stage < loop_stage) {
        ...
        for (size_t vl; n > 0; n -= vl, src_llr_ptr += vl*2, dst_llr_ptr += vl) {
            ...
            vfloat32m1_t llr0 = __riscv_vreinterpret_f32m1(__riscv_vnsrl(llr, 0, vl));
            vfloat32m1_t llr1 = __riscv_vreinterpret_f32m1(__riscv_vnsrl(llr, 32, vl));
            vfloat32m1_t v = __riscv_vfmin(__riscv_vfabs(llr0, vl),
                                           __riscv_vfabs(llr1, vl), vl);
...
static inline void
volk_32f_8u_polarbutterfly_32f_rvvseg(float* llrs,
                                      unsigned char* u,
                                      const int frame_exp,
                                      const int stage,
                                      const int u_num,
