...
static inline void
volk_32f_x2_interleave_32fc_rvv(lv_32fc_t* complexVector,
                                const float* iBuffer,
                                const float* qBuffer,
                                unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, out += vl, iBuffer += vl, qBuffer += vl) {
        ...
        vuint64m8_t vc = __riscv_vwmaccu(__riscv_vwaddu_vv(vr, vi, vl), 0xFFFFFFFF, vi, vl);
...
static inline void
volk_32f_x2_interleave_32fc_rvvseg(lv_32fc_t* complexVector,
                                   const float* iBuffer,
                                   const float* qBuffer,
                                   unsigned int num_points)
{
