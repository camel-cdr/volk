...
static inline void
volk_32fc_s32f_power_spectrum_32f_rvv(float* logPowerOutput,
                                      const lv_32fc_t* complexFFTInput,
                                      const float normalizationFactor,
                                      unsigned int num_points)
{
...
#error
    ...
    for (size_t vl; n > 0; n -= vl, complexFFTInput += vl, logPowerOutput += vl) {
        ...
        vfloat32m2_t exp = __riscv_vfcvt_f(__riscv_vsub(
                 __riscv_vsra(__riscv_vreinterpret_i32m2(a), 23, vl), c127, vl), vl);
        vfloat32m2_t frac = __riscv_vreinterpret_f32m2(__riscv_vor(__riscv_vand(
                __riscv_vreinterpret_i32m2(v), m2, vl), m1, vl));
...
#error
    ...
    for (size_t vl; n > 0; n -= vl, complexFFTInput += vl, logPowerOutput += vl) {
        vl = __riscv_vsetvl_e32m2(n);
        vfloat32m2x2_t vc = __riscv_vlseg2e32_v_f32m2x2((const float*)complexFFTInput  , vl);
