...
static inline void
volk_16u_byteswap_rvv(uint16_t* intsToSwap,
                      unsigned int num_points)
{
    size_t n = num_points;
    ...
    if (vlmax <= 256) {
        vuint8m1_t vidx = __riscv_vreinterpret_u8m1(__riscv_vsub(
                __riscv_vreinterpret_u16m1(__riscv_vid_v_u8m1(vlmax)), 0x100-0x1, vlmax/2));
        for (size_t vl; n > 0; n -= vl, intsToSwap += vl) {
            vl = __riscv_vsetvl_e16m8(n);
            vuint8m8_t v = __riscv_vreinterpret_u8m8(__riscv_vle16_v_u16m8(intsToSwap, vl));
        ...
        vuint16m2_t vidx = __riscv_vreinterpret_u16m2(__riscv_vsub(
                __riscv_vreinterpret_u32m2(__riscv_vid_v_u16m2(vlmax)), 0x10000-0x1, vlmax/2));
        for (size_t vl; n > 0; n -= vl, intsToSwap += vl) {
            vl = __riscv_vsetvl_e16m8(n);
            vuint8m8_t v = __riscv_vreinterpret_u8m8(__riscv_vle16_v_u16m8(intsToSwap, vl));
...
static inline void
volk_16u_byteswap_rva23(uint16_t* intsToSwap,
                        unsigned int num_points)
{
    size_t n = num_points;
    for (size_t vl; n > 0; n -= vl, intsToSwap += vl) {
