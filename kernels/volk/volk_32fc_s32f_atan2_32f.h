...
static inline void
volk_32fc_s32f_atan2_32f_rvv(float* outputVector,
                             const lv_32fc_t* inputVector,
                             const float normalizeFactor,
                             unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, inputVector += vl, outputVector += vl) {
        ...
        vfloat32m2_t x = __riscv_vfdiv(__riscv_vmerge(vi, vr, mswap, vl),
                                       __riscv_vmerge(vr, vi, mswap, vl), vl);
        ...
        p = __riscv_vfadd_mu(RISCV_VMFLTZ(32m2, vr, vl), p, p, __riscv_vfsgnjx(cpi, vi, vl), vl);
...
static inline void
volk_32fc_s32f_atan2_32f_rvvseg(float* outputVector,
                                const lv_32fc_t* inputVector,
                                const float normalizeFactor,
                                unsigned int num_points)
{
