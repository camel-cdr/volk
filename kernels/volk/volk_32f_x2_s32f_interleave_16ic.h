...
static inline void
volk_32f_x2_s32f_interleave_16ic_rvv(lv_16sc_t* complexVector,
                                     const float* iBuffer,
                                     const float* qBuffer,
                                     const float scalar,
                                     unsigned int num_points)
...
static inline void
volk_32f_x2_s32f_interleave_16ic_rvvseg(lv_16sc_t* complexVector,
                                        const float* iBuffer,
                                        const float* qBuffer,
                                        const float scalar,
                                        unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, complexVector += vl, iBuffer += vl, qBuffer += vl) {
        ...
        vint16m4_t vri = __riscv_vfncvt_x(__riscv_vfmul(vrf, scalar, vl), vl);
        vint16m4_t vii = __riscv_vfncvt_x(__riscv_vfmul(vif, scalar, vl), vl);
        __riscv_vsseg2e16((int16_t*)complexVector, __riscv_vcreate_v_i16m4x2(vri, vii), vl);
    }
}
