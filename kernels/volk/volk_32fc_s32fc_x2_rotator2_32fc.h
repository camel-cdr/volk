...
static inline void
volk_32fc_s32fc_x2_rotator2_32fc_rvv(lv_32fc_t* outVector,
                                     const lv_32fc_t* inVector,
                                     const lv_32fc_t* phase_inc,
                                     lv_32fc_t* phase,
                                     unsigned int num_points)
{
    ...
    for (size_t i = 0; i < vlmax; ++i) {
        lv_32fc_t ph = lv_cmake(lv_creal(*phase)*lv_creal(inc) - lv_cimag(*phase)*lv_cimag(inc),
        ...
        inc = lv_cmake(lv_creal(*phase_inc)*lv_creal(inc) - lv_cimag(*phase_inc)*lv_cimag(inc),
    ...
    if (num_points > 0) while (1) {
        ...
        for (; n > 0; n -= vl, inVector += vl, outVector += vl) {
            ...
            vfloat32m2_t vr = __riscv_vfnmsac(__riscv_vfmul(var, phr, vl), vai, phi, vl);
            vfloat32m2_t vi = __riscv_vfmacc( __riscv_vfmul(var, phi, vl), vai, phr, vl);

            vuint32m2_t vru = __riscv_vreinterpret_u32m2(vr);
            vuint32m2_t viu = __riscv_vreinterpret_u32m2(vi);
            vuint64m4_t res = __riscv_vwmaccu(__riscv_vwaddu_vv(vru, viu, vl), 0xFFFFFFFF, viu, vl);
        ...
        // normalize
        vfloat32m2_t scale = __riscv_vfmacc(__riscv_vfmul(phr, phr, vl), phi, phi, vl);
...
static inline void
volk_32fc_s32fc_x2_rotator2_32fc_rvvseg(lv_32fc_t* outVector,
                                        const lv_32fc_t* inVector,
                                        const lv_32fc_t* phase_inc,
                                        lv_32fc_t* phase,
                                        unsigned int num_points)
{
    ...
    if (num_points > 0) while (1) {
        ...
        for (; n > 0; n -= vl, inVector += vl, outVector += vl) {
            ...
            vfloat32m2x2_t va = __riscv_vlseg2e32_v_f32m2x2((const float*)inVector, vl);
