...
static inline void
volk_32fc_x2_divide_32fc_rvv(lv_32fc_t* cVector,
                             const lv_32fc_t* aVector,
                             const lv_32fc_t* bVector,
                             unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, aVector += vl, bVector += vl, out += vl) {
        ...
        vfloat32m4_t mul = __riscv_vfrdiv(__riscv_vfmacc(__riscv_vfmul(vbi, vbi, vl), vbr, vbr, vl), 1.0f, vl);
        vfloat32m4_t vr = __riscv_vfmul(__riscv_vfmacc( __riscv_vfmul(var, vbr, vl), vai, vbi, vl), mul, vl);
        vfloat32m4_t vi = __riscv_vfmul(__riscv_vfnmsac(__riscv_vfmul(vai, vbr, vl), var, vbi, vl), mul, vl);
        vuint32m4_t vru = __riscv_vreinterpret_u32m4(vr);
        vuint32m4_t viu = __riscv_vreinterpret_u32m4(vi);
        vuint64m8_t v = __riscv_vwmaccu(__riscv_vwaddu_vv(vru, viu, vl), 0xFFFFFFFF, viu, vl);
...
static inline void
volk_32fc_x2_divide_32fc_rvvseg(lv_32fc_t* cVector,
                                const lv_32fc_t* aVector,
                                const lv_32fc_t* bVector,
                                unsigned int num_points)
{
    ...
    for (size_t vl; n > 0; n -= vl, aVector += vl, bVector += vl, cVector += vl) {
        ...
        vfloat32m4_t vr = __riscv_vfmul(__riscv_vfmacc( __riscv_vfmul(var, vbr, vl), vai, vbi, vl), mul, vl);
        vfloat32m4_t vi = __riscv_vfmul(__riscv_vfnmsac(__riscv_vfmul(vai, vbr, vl), var, vbi, vl), mul, vl);
        __riscv_vsseg2e32_v_f32m4x2((float*)cVector, __riscv_vcreate_v_f32m4x2(vr, vi), vl);
    }
}
